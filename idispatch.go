...
func invoke(disp *IDispatch, dispid int32, dispatch int16, params ...interface{}) (result *VARIANT, err error) {
	...
	if len(params) > 0 {
		...
		for i, v := range params {
			...
			case uint16:
				vargs[n] = VARIANT{VT_UI2, 0, 0, 0, int64(v.(uint16))}
			case *uint16:
				vargs[n] = VARIANT{VT_UI2 | VT_BYREF, 0, 0, 0, int64(uintptr(unsafe.Pointer(v.(*uint16))))}
			case int, int32:
				vargs[n] = VARIANT{VT_I4, 0, 0, 0, int64(v.(int))}
			case *int, *int32:
			...
			case string:
				vargs[n] = VARIANT{VT_BSTR, 0, 0, 0, int64(uintptr(unsafe.Pointer(SysAllocStringLen(v.(string)))))}
			case *string:
			...
			case []byte:
				...
				vargs[n] = VARIANT{VT_ARRAY | VT_UI1, 0, 0, 0, int64(uintptr(unsafe.Pointer(safeByteArray)))}
